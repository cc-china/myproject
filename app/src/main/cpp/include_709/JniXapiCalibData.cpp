...
static jobject nativeReadCalibData(JNIEnv *env, jobject thiz, jobject param)
{
        ...
        if (ioRet == size) {
            ...
            if (PIOK == pi_ret) {
                ...
                LOGI("right %lf %lf %lf %lf %lf %lf %lf %lf %lf",
                        ...
                        hyn.right.k3);
                for (int i = 0; i < 9; ++i)
                    LOGI("R[%d]:%lf", i, hyn.R[i]);
                for (int i = 0; i < 3; ++i)
                    LOGI("T[%d]:%lf", i, hyn.T[i]);
#endif
                //float2double
    ...
    if (pData)
        free(pData);
out:
    LOGE("nativeReadDepthResult ret=%d ioRet=%d", ret, ioRet);
    if (ret)
        return resultObj;
    else
        return NULL;
...
static jobject nativeReadCalibData(JNIEnv *env, jobject thiz, jobject param)
{
    ...
    if (fd >= 0 && pData) {
        ...
        LOGI("nativeReadDepthResult error fd=%d, pData=%p", fd, pData);
    }
    if (pData)
        free(pData);
out:
   LOGI("nativeReadDepthResult ret=%d ioRet=%d", ret, ioRet);
    if (ret)
        return resultObj;
    else
        return NULL;
